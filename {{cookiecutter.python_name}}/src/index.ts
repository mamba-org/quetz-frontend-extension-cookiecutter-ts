import {
  QuetzFrontEnd,
  QuetzFrontEndPlugin
} from '@quetz-frontend/application';{% if cookiecutter.kind.lower() == 'theme' %}
...
const plugin: QuetzFrontEndPlugin<void> = {
  ...
  activate: (app: QuetzFrontEnd{% if cookiecutter.kind.lower() == 'theme' %}, manager: IThemeManager{% endif %}{% if cookiecutter.has_settings.lower().startswith('y') %}, settingRegistry: ISettingRegistry | null{% endif %}) => {
